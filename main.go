...
Domain Expiration Monitoring and Alert System
...
var (
	domains        = []string{"www.cheq.one", "api.cheq.one", "console.cheq.one", "content.cheq.one", "devcontent.cheq.one", "grafana.cheq.one", "uat.cheq.one", "uatconsole.cheq.one"}
...
func sendMail(expiringDomains []DomainInfo, mailType string) {
	...
	recipients := []string{"er.learning14@gmail.com"} //, "vdagwar@moreyeahs.com"
